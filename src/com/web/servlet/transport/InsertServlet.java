...
//@WebServlet用于配置当前的请求访问的地址  /Insert
@WebServlet("/transportInsert")
public class InsertServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        Manufacturer okk = mDao.Query_M_id(m_id);
        if (okk == null) {
            result.put("message", "信息修改失败,该\"m_id:"+m_id+"\"还未在生产商上链");
            //写出结果
            response.getWriter().append(gson.toJson(result));
            return;//结束方法
        }
        ...
        //构建Transport 对象
        Transport t = new Transport(null, null, t_name, t_operator,
        ...
        boolean ok = dao.insertTransport(t);
        // 3、判断是否修改成功
        if (ok) {
            result.put("ok", true);
        } else {
            result.put("ok", false);
            result.put("message", "信息修改失败,该编号的运输商信息已存在");
        }
        //3、响应 --写出结果
        response.getWriter().append(gson.toJson(result));
    }

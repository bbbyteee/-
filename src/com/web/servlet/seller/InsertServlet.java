...
public class InsertServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        Transport okk = tDao.Query_M_id(m_id);
        if (okk == null) {
            result.put("message", "信息修改失败,该编号:"+m_id+" 还未在运输商上链");
            //写出结果
            response.getWriter().append(gson.toJson(result));
            return;//结束方法
        }
        //构建Seller 对象
        Seller s = new Seller(null, null, s_seller,
                s_COVID19_Report,s_in_date,s_out_date,s_launch_date, u_id, m_id);
        // 2、创建dao 的对象，然后调佣dao方法
        SellerDao dao = new SellerDao();
        boolean ok = dao.insertSeller(s);
        // 3、判断是否修改成功
        if (ok) {
            result.put("ok", true);
        } else {
            result.put("ok", false);
            result.put("message", "信息修改失败，该编号的销售商信息已存在");
        }
        //3、响应 --写出结果
        response.getWriter().append(gson.toJson(result));
    }
